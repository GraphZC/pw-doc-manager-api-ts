...
export const InvoiceFindFirstSchema = Joi.object().keys({ where: Joi.object().keys(InvoiceWhereInputSchemaObject), orderBy: Joi.object().keys(InvoiceOrderByWithRelationInputSchemaObject), cursor: Joi.object().keys(InvoiceWhereUniqueInputSchemaObject), take: Joi.number(), skip: Joi.number(), distinct: Joi.array().items(InvoiceScalarFieldEnumSchema) }).required()